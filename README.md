...
1. Install [SuperTuxKart 1.5 for macOS](https://github.com/supertuxkart/stk-code/releases/tag/1.5) (not needed for the one-badge proof below).
...
Do not enable Terminal in place of `BadgeKartBridge.app`. `start.command` only resigns the app when the binary actually changed, so TCC can remember it.

## Two-minute one-badge proof

Do this on the Mac with one badge. The cloud agent cannot see USB hardware.
...
5. Shake once. Expect one `SHAKE` / `N DOWN` then `N UP`. A second shake inside ~0.8s is ignored.
6. Hold **A**, unplug the badge. Expect `releasing A (disconnect)` and `all keys UP`.
...
`--demo` from `swift run` is a Terminal child process. Use `start.command` (the installed `.app`) for the real Accessibility identity. `start.command --demo` launches the same canned P1/P2 sequence through the `.app`.

Run automated parser and mapping tests:
...
```sh
# after start.command has installed the app:
